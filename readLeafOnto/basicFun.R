calcR2 <- function(res, val, average){
  r2 <- 1-(sum(res^2)/sum((val-average)^2))
  return(r2)
}
calcRMSE <- function(val, predictions, sampleSize){
  RMSE <- sqrt(sum((val-predictions)^2)/sampleSize)
  return(RMSE)
}
calcVPD <- function(RH, temp){0.61365 * exp(17.502 * temp/(240.97 + temp)) * (1 -(RH/100))}
noNAlength <- function(x){length(x)-length(which(is.na(x)))}
rmDup <- function(dfr, whichvar){  
  dfr <- dfr[!duplicated(dfr[,whichvar]),]
  return(dfr)
}
s.err <- function(x){sd(x, na.rm = TRUE)/sqrt(noNAlength(x))}
mean.na <- function(x){mean(x, na.rm = TRUE)}
max.na <- function(x){max(x, na.rm = TRUE)}
convertPAR <- function(x){
  conv <- sum(x, na.rm=TRUE)*10*60/1000000
  return(conv)
}


quantile5 <- function(x){quantile(x,0.05, na.rm=TRUE)}
quantile95 <- function(x){quantile(x,0.95, na.rm=TRUE)}
